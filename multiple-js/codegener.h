...
struct Scope {
	...
	std::map<std::string, uint32_t> var_table;		// 变量表，key为变量索引
};
...
private:
	...
	// 常量
	std::map<Value*, uint32_t> const_map_;		// 暂时有问题，指针就没办法找重载<了
	...
	// 循环
	uint32_t cur_loop_start_pc_;
	std::vector<uint32_t>* cur_loop_repair_end_pc_list_;
};
