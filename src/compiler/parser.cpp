...
std::unique_ptr<Expression> Parser::ParseUnaryExpression() {
	...
	case TokenType::kKwAwait: {
		lexer_->NextToken();
		exp = std::make_unique<UnaryExpression>(start, lexer_->GetSourcePos(), token.type(), ParseUnaryExpression());
		break;
	}
	case TokenType::kOpInc: {
		...
		exp = std::make_unique<UnaryExpression>(start, lexer_->GetSourcePos(), TokenType::kOpPrefixInc, std::move(exp));
		break;
	}
	case TokenType::kOpDec: {
		...
		exp = std::make_unique<UnaryExpression>(start, lexer_->GetSourcePos(), TokenType::kOpPrefixDec, std::move(exp));
		break;
	}
	default:
