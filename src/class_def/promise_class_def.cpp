...
{
	property_map_.emplace(runtime, "then", Value([](Context* context, uint32_t par_count, const StackFrame& stack) -> Value {
	...
	static_property_map_.emplace(runtime, "resolve", Value([](Context* context, uint32_t par_count, const StackFrame& stack) -> Value {
		Value value;
		if (par_count > 0) {
			value = stack.get(0);
		}
	...
	static_property_map_.emplace(runtime, "reject", Value([](Context* context, uint32_t par_count, const StackFrame& stack) -> Value {
		return Value();
	}));
}
